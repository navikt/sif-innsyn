...
async function getTokenXToken(token, additionalClaims, useAudienceK9BrukerdialogApi) {
    let tokenSet;
    try {
        tokenSet = await tokenxClient?.grant(
            {
                ...
                audience:
                    useAudienceK9BrukerdialogApi === true
                        ? process.env.K9_BRUKERDIALOG_API_TOKENX_AUDIENCE
                        : process.env.API_TOKENX_AUDIENCE,
                subject_token: token,
            },
            additionalClaims
        );
...
async function exchangeToken(token, useAudienceK9BrukerdialogApi) {
    if (!token) {
    ...
    return await getTokenXToken(token, additionalClaims, useAudienceK9BrukerdialogApi);
}
